...
for model in models:
    ...
    # Soil temperature
    tsl_path = glob.glob(
        f"../Data/Moisture_driven/Soil_temperature/tsl_{model}_historical_1982-2014_CoLM_vertical_pf_region.nc"
    )[0]

    # Soil heat flux at the upper boundary
    hs_path = glob.glob(f"../Data/Moisture_driven/hs/{model}_*.nc")[0]
    ...
    for v in ['theta_s','psi_s','tkdry','tksatf','tksatu']:
        ds = xr.open_dataset(f"../Data/Moisture_driven/soil_parameter/{v}_adjusted_{model}.nc")
        ds_param[v] = ds[v].transpose("layer","lat","lon").values

    # remapcon
    for v in ['csol','lambda','vf_sand_s','vf_om_s','vf_gravels_s']:
        ds = xr.open_dataset(f"../Data/Moisture_driven/soil_parameter/{v}_remapcon_{model}.nc")
        ds_param[v] = ds[v].transpose("layer","lat","lon").values
    ...
    porsl_15      = np.zeros((15, nlat, nlon))
    ...
    def run_one_pixel(m, n):
        ...
        # ========== Assign parameters ==========
        porsl_15[:bedrock_layer,m,n] = porsl_10[:bedrock_layer,m,n]
        porsl = porsl_15[:, m, n]
